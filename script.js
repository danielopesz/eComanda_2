...
(() => {
    ...
    var require_webflow_tabs = __commonJS({
      "packages/shared/render/plugins/Tabs/webflow-tabs.js"(exports, module) {
        ...
        document.querySelectorAll('.process-tab-button').forEach(button => {
            button.addEventListener('click', () => {
                const tabId = button.getAttribute('data-w-tab');
                
                // Remover classes 'active' de todos os botões e abas
                document.querySelectorAll('.process-tab-button').forEach(btn => btn.classList.remove('w--current'));
                document.querySelectorAll('.process-tab-pane').forEach(pane => pane.classList.remove('w--tab-active'));
                
                // Adicionar 'active' à aba e botão selecionados
                button.classList.add('w--current');
                document.querySelector(`.process-tab-pane[data-w-tab="${tabId}"]`).classList.add('w--tab-active');
            });
        });
        // Função para manipular o clique nas abas
        document.querySelectorAll('.tab-button-left, .tab-button-right').forEach(button => {
            button.addEventListener('click', () => {
                const tabId = button.getAttribute('data-w-tab');
                
                // Remover classes 'active' de todos os botões e abas
                document.querySelectorAll('.tab-button-left, .tab-button-right').forEach(btn => btn.classList.remove('w--current'));
                document.querySelectorAll('.pricing-tab-pane').forEach(pane => pane.classList.remove('w--tab-active'));
                
                // Adicionar 'active' à aba e botão selecionados
                button.classList.add('w--current');
                document.querySelector(`.pricing-tab-pane[data-w-tab="${tabId}"]`).classList.add('w--tab-active');
            });
        });
